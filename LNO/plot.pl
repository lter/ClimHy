...
sub counter {
	my $method = shift;
	my ($count_a,$count_b);
	open COUNTER, "<$Bin/log/counter" or die "Can't open counter file";
	while (<COUNTER>) {
		($count_a,$count_b) = split /,/,$_;}
	close COUNTER;
	open COUNTER, ">$Bin/log/counter" or die "Can't write to counter file";
	if ($method) {
		$count_b++;
	} else {
		$count_a++;}
...
# if they want to see it
	...
	if ($method =~ /plot/) {
		...
		print $cgi->redirect("temp/plot_image.html");
	} else {
...
sub mk_plot_html {
	...
	my $plot_image = "$Bin/temp/plot_image.html";
...
sub print_data {
	...
	my $i = 1;
	my $flag = '<TH>Estimates</TH><TH>Observations</TH>';
...
#JAMES ADDED
 ...
 print $cgi->header(-expires=>'now'),
		$cgi->start_html(-title=>'Page 4',-author=>'kyle.kotwica@comcast.net',-style=>{'src'=>"$style"}); }
...
sub exec_query {
	...
	if ( (!$file_1) && ($method eq 'plot') ) {
		#($data1_fh,$file_1) = tempfile(UNLINK => 1, DIR => "$Bin\\temp");
		#($data1_fh,$file_1) = tempfile(UNLINK => 1, DIR => "$Bin/temp");
		($data1_fh,$file_1) = tempfile(DIR => "$Bin/temp");
		...
		if ($method eq 'plot') {
			...
			if ($agg_period =~ /monthly/) {
				...
				for ($i=4; $i<$size; $i=$i+4) {
					if (defined $$data[$i]) {
						if (!$$data[$i+3]) {$$data[$i+3] = 3;}
						$_ = $_ . sprintf "\t%8.$$data[$i+3]f\t%d\t%d",$$data[$i],$$data[$i+1],$$data[$i+2];
					} else {
						$_ = $_ ."\t \t \t ";
					}
				}
...
# q gets ignored.
	...
	($gif_fh,$gnu_gif) = tempfile(SUFFIX => '.png', DIR => "$Bin/temp");
...
sub upload {
	my $method = shift;
		if ($method =~ /comma/) {
print $cgi->header(-nph=>1,-type=>'text/csv',-attachment=>'data.csv');
#			print "Content-type: application/vnd.ms-excel\nContent-Disposition: file; filename=\"data.csv\"\n\n";
###			print "Content-type: text/csv\nContent-Disposition: attachment; filename=\"data.csv\"\n\n";
		}
		if ($method =~ /tab/) {
print $cgi->header(-nph=>1,-type=>'text/tab-separated-values',-attachment=>'data.dat');
