...
if (ref(\$som) eq 'HASH') {
	...
	my \$huh = \$som->valueof("//Envelope/Body/".\$method."Response/complex/row/");
	my \$rows = values (%{$huh});
	my \@list = \$som->valueof("//Envelope/Body/".\$method."Response/complex/row/*");
	while (\$list[0]) {
		my \@short_list = splice(\@list,0,\$rows);
...
# foreach method
	for(my $i=0;$i<=$self->number;$i++) {
		...
		my $module = $self->methods->[$i]->module;
		my @h_names = @{$self->methods->[$i]->response_names};
		my $repeat = (@h_names/2);

		if ($DSN) {
			$guts = "my \@result = \&query_DB(\"$sqlcmd\",\'$DSN\',\'$user\',\'$password\', \@_);";
		} else {
			$guts = "use $module;\n\tmy \@result = \&$module(\@_);";
		}
...
;
			while (my($elem_name,$elem_type) = splice @h_names,0,2) {
				print HANDLER "\t\tSOAP::Data->name(\"$elem_name\"=>\$result[\$i+$j]),\n";
				$j++;
			}
...
require Exporter;
@ISA = qw(ObjectTemplate Exporter);
attributes qw(method request response request_names response_names rows sqlcmd DSN user password database name module);
