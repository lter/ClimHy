...
#
###
### This code (D:/forwww/lter/climdb/wambam/services/climdb_agg_handler.pl) has been automagicaly created by D:\forwww\lter\climdb\wambam\soap_server.pl at 1104785366
...
sub get_agg_monthly {
	...
	my $som;
	my @soms;
	my @result = &query_DB("SELECT site_code, station, variable, aggregate_type, year, month, value FROM climdb_agg WHERE site_code =  ? and station = ? and variable = ? and aggregate_type =  ? and convert(int,year) >= ? and convert(int,year) <= ? ","DBI:ODBC:rocky_lter_climdb","climdb","adm4cdb*",@_);
...
sub get_agg_yearly {
	...
	my $som;
	my @soms;
	my @result = &query_DB("SELECT site_code, station, variable, aggregate_type, year, value FROM climdb_agg WHERE site_code = ? and station = ? and variable = ? and aggregate_type = ? and convert(int,year) >= ? and convert(int,year) <= ? and month = 99","DBI:ODBC:rocky_lter_climdb","climdb","adm4cdb*",@_);
...
sub agg_over_all {
	...
	my $som;
	my @soms;
	my @result = &query_DB("SELECT site_code, station, variable, aggregate_type, year, value FROM climdb_agg WHERE site_code = ? and station = ? and variable = ? and aggregate_type = ? and year = 0","DBI:ODBC:rocky_lter_climdb","climdb","adm4cdb*",@_);
