...
#
###
### This code (E:/lter/climhy/wambam/services/climdb_raw_handler.pl) has been automagicaly created by E:\lter\climhy\wambam\soap_server.pl at 1108084747
...
sub get_day {
	...
	my $som;
	my @soms;
	my @result = &query_DB("select variable, value, flag from climdb_raw where res_site_id = ? and sampledate = ?","DBI:ODBC:rocky_lter_climdb","climdb","adm4cdb*",@_);
...
sub last_harvest {
	...
	my $som;
	my @soms;
	BEGIN {unshift (@INC, 'E:\\lter\\climhy\\wambam\\bin\\' ) };
	use last_harvest;
...
sub get_variable {
	...
	my $som;
	my @soms;
	my @result = &query_DB("SELECT variable,value,flag,sampledate from climdb_raw where site_code = ? and station = ? and variable = ? and sampledate>= ? and sampledate <=  ?","DBI:ODBC:rocky_lter_climdb","climdb","adm4cdb*",@_);
...
sub logger {
	...
	open LOGGER, ">>E:/lter/climhy/wambam//log/log.log" or die;
