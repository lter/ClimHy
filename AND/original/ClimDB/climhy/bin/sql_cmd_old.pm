...
# SQL commands for plot.pl
	...
	if ($which[0] =~ /^300mm$/) {
		...
		for (my $i=0;$i<=$#var_list;$i++) {
			$sqlcmd .= ",temp$i.value, temp$i.num_e, temp$i.decimals";
		}
		$sqlcmd .= "\nFROM temp_dates\n";
		for (my $i=0;$i<=$#var_list;$i++) {
	...
	if ($which[0] =~ /^300yy$/) {
		...
		$sqlcmd .= "\n@@@#MAKE A TEMP TABLE FOR EACH VARIABLE (temp(n))\n"; 
		for (my $i=0;$i<=$#var_list;$i++) {
			$sqlcmd .= "@@@ SELECT site_code, station, year, value, num_e, v.decimals\nINTO temp$i\nFROM climdb_agg a, climdb_variables v\nWHERE site_code='$site' and\n\t station='$station' and\n\t a.variable='$var_list[$i]' and\n\t year>='$begin' and\n\t year<='$end' and\n\t aggregate_type = '$agg_list[$i]' AND \n\t month = 99 AND v.variable = a.variable \n";
		}
		$sqlcmd .= "\n@@@#DO A UNION ON ALL temp(n)s IN ORDER TO HOLD THE MIN AND MAX DATES\n"; 
		$sqlcmd .= "@@@ SELECT min(year) as mindate, max(year) as maxdate into temp_union FROM temp0\n";
		for (my $i=1;$i<=$#var_list;$i++) {
...
VALUES
('$session_id',getdate(),'$ip_address','$requestor','$organization','$purpose')";
		last SWITCH; }
...
(session_id,download_id,datetime,ip_address,file_type)
VALUES
('$session_id','$download_id',getdate(),'$ip_address','$file_type')";
		last SWITCH; }
