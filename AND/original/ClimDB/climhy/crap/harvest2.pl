...
# shared with all of the FUNDME modules and keeps variables that may change 
# due to porting and what not. Make sure you can find climdb.conf!
# climdb.conf (and all of the modules that are included with this package)
# should be in the directory listed in the next command.

BEGIN {unshift (@INC, 'D:\forwww\lter\climhy\bin' ) }
...
<P>Option 1<input type=radio name=module value=1 CHECKED> (Original ClimDB URL)
<BR>Option 2<input type=radio name=module value=2> (Original HydroDB URL)
<BR>Option 3<input type=radio name=module value=3> (USGS data harvest)
...
# query data base for modification times ? 
	...
	print LOGIT ("\n\nWould you like to return to the <A HREF='http://www.fsl.orst.edu/$lower_name/harvest.htm'>$module_name participant web page</A>?<BR>\n");
