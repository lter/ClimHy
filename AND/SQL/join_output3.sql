/* select distinct sampledate into masterdate from climdb_raw order by sampledate */
...
select min(sampledate) as mindate,max(sampledate) as maxdate into #temp5 from #temp2
  union
  select min(sampledate) as mindate,max(sampledate) as maxdate from #temp3
  union
  select min(sampledate) as mindate,max(sampledate) as maxdate from #temp4
select 'AND' as site_code, 'PRIMET' as station,sampledate
 into #temp1
 from masterdate
 where sampledate>=(select min(mindate) from #temp5) and 
       sampledate<=(select max(maxdate) from #temp5)
